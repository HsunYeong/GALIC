      ...
      for(j = 0; j < n; j++)
        {
          ...
          double sigma_star = All.Disk_Mass / (2 * M_PI * All.Disk_R * All.Disk_R) * exp(-R / All.Disk_R);
...
void output_rotcurve_avg(void)
{
  if(ThisTask == 0)
    {
      double pos[3], R, acc[3], Z;
      double disp_r, disp_t, disp_p, disp_q;
      double disp;
      double vphi;
      double rho;
