...
void forcedensitygrid_create(void) {
	...
	fac = All.OutermostBinEnclosedMassFraction;
        ...
        if (All.HaloUseTable == 0)   All.Rmax = All.Halo_A * (fac + sqrt(fac)) / (1 - fac);

	else All.Rmax = halo_mass_to_radius(fac*All.Halo_Mass);

	mpi_printf("\nGrid structure:\n" "Rmax = %g  (outer edge of grid)\n", All.Rmax);

	fac = All.InnermostBinEnclosedMassFraction;

        if (All.HaloUseTable == 0)   DG_Rin = All.Halo_A * (fac + sqrt(fac)) / (1 - fac);
	  ...
	  while(count < ntarget)
	    {
	      if(type == 1)
                ...
                bulge_get_fresh_coordinate(pos);/* bulge particle */
...
void force_test(void)
{
  ...
  if(ThisTask == 0)
    {
      char buf[1000];
      sprintf(buf, "%s/forcetest.dat", All.OutputDir);
      FILE *fd = fopen(buf, "w");
